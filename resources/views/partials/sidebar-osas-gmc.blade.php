<div class="flex flex-col h-full">
    <nav class="flex flex-col gap-0 flex-1">
        ...
        <a href="{{ route('osas-gmc.dashboard') }}"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="{{ isset($sidebarCollapsed) && $sidebarCollapsed ? 'w-8 h-8' : 'w-6 h-6' }} flex-shrink-0" fill="none" stroke="currentColor">
        ...
        <!-- Requests -->
        <details class="group" {{ request()->routeIs('osas-gmc.requests') ? 'open' : '' }}>
            <summary class="flex items-center gap-3 px-6 py-4 font-medium cursor-pointer {{ request()->routeIs('osas-gmc.requests') ? 'bg-red-900 text-white' : 'text-red-900 hover:bg-red-800 hover:text-white' }} transition-colors rounded-r-lg w-full list-none">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="{{ isset($sidebarCollapsed) && $sidebarCollapsed ? 'w-8 h-8' : 'w-6 h-6' }} flex-shrink-0" fill="none" stroke="currentColor">
            ...
            <div class="sidebar-text bg-red-50 bg-opacity-10">
                <a href="{{ route('osas-gmc.requests', ['tab' => 'goodmoral']) }}" class="flex items-center py-3 pl-[60px] pr-6 text-sm {{ request('tab') != 'safeloan' ? 'bg-red-900 text-white' : 'text-red-900 hover:bg-red-800 hover:text-white' }} transition-colors">
                    Good Moral
                </a>
                <a href="{{ route('osas-gmc.requests', ['tab' => 'safeloan']) }}" class="flex items-center py-3 pl-[60px] pr-6 text-sm {{ request('tab') == 'safeloan' ? 'bg-red-900 text-white' : 'text-red-900 hover:bg-red-800 hover:text-white' }} transition-colors">
                    Safe Loan
                </a>
            </div>
        </details>
