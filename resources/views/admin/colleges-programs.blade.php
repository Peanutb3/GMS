...
<div class="mb-6">
    <div class="bg-white rounded-lg shadow-sm p-4">
        <div class="relative">
            <input type="text" id="searchInput" placeholder="Search colleges or programs..."
                class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-800 focus:border-transparent"
                oninput="filterCollegesAndPrograms()">
...
<!-- Colleges List -->
<div class="space-y-6" id="collegesList">
    @forelse($colleges as $college)
    <div class="bg-white rounded-xl shadow-lg overflow-hidden college-item" data-college-name="{{ strtolower($college->name) }}" data-college-code="{{ strtolower($college->code ?? '') }}">
        <!-- College Header -->
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 program-row" data-program-name="{{ strtolower($program->name) }}" data-program-code="{{ strtolower($program->code ?? '') }}">
                            <td class="px-4 py-3">
...
<script>
    ...
    function filterCollegesAndPrograms() {
        const searchTerm = document.getElementById('searchInput').value.toLowerCase();
        const collegeItems = document.querySelectorAll('.college-item');
        let visibleCount = 0;

        collegeItems.forEach(college => {
            const collegeName = college.getAttribute('data-college-name');
            const collegeCode = college.getAttribute('data-college-code');
            const programRows = college.querySelectorAll('.program-row');

            // Check if college matches
            const collegeMatches = collegeName.includes(searchTerm) || collegeCode.includes(searchTerm);

            // Check and filter programs
            let hasVisiblePrograms = false;
            programRows.forEach(programRow => {
                const programName = programRow.getAttribute('data-program-name');
                const programCode = programRow.getAttribute('data-program-code');
                const programMatches = programName.includes(searchTerm) || programCode.includes(searchTerm);

                if (programMatches || collegeMatches) {
            ...
            // Show college if it matches or has visible programs
            if (collegeMatches || hasVisiblePrograms) {
        ...
        if (visibleCount === 0 && searchTerm !== '') {
            if (!noResultsMsg) {
                ...
                noResultsMsg.className = 'text-center py-12 text-gray-400';
                ...
                `;
                collegesList.appendChild(noResultsMsg);
            }
